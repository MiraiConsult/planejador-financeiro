    ...
    >
      ...
      <div
        className={cn(
          ...
          selected ? 'border-brand-600 bg-brand-600' : 'border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800',
        )}
      >
...
export function EmptyState({ icon: Icon, title, description, action }: { icon: React.ComponentType<{ size?: number; className?: string }>; title: string; description: string; action?: ReactNode }) {
  return (
    <div className="rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 p-10 text-center bg-slate-50/30 dark:bg-slate-800/30">
      <div className="mx-auto mb-4 h-12 w-12 rounded-xl bg-white dark:bg-slate-900 shadow-sm dark:shadow-none flex items-center justify-center text-slate-400 dark:text-slate-500">
        <Icon size={20} />
      </div>
